...
Step-by-step
1) Convert integer tokens (patch indices) to dense vectors
...
def run_example(batch_size=4, seq_len=16, vocab_size=1000):
    ...
    with torch.no_grad():
        out = model(x)
    print("ViT-like Encoder Example")
    print(f"Input ids: {tuple(x.shape)}")
    print(f"Output (logits): {tuple(out.shape)}")
    print("Diagram: [Patch tokens] -> [Embedding] -> [Self-Attn x N] -> [Pool] -> [Linear]")
